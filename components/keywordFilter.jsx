...
class KeywordFilter extends React.Component {
  ...
  onKeyPress(e) {
    if (e.key === 'Enter') {
      const newSearchWords = this.state.searchWords.concat(e.target.value);
  ...
  onKeyWordRemoval(keyword) {
    const newSearchWords = this.state.searchWords.filter(kw => kw != keyword);
    this.setState({ searchWords: newSearchWords });
    this.props.keywordsChanged(newSearchWords);
  }
  ...
  render() {
    const keywordNodes = this.state.searchWords.map((keyword, i) => {
      return (
...
KeywordFilter.propTypes = {
  keywordsChanged: React.PropTypes.func.isRequired,
};
